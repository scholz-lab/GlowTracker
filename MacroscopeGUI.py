...
class RuntimeControls(BoxLayout):
    ...
    def tracking(self, minstep, units, capture_radius, binning, dark_bg):
        """thread for tracking"""
        app = App.get_running_app()
        stage = app.stage
        camera = app.camera
        ...
        while camera is not None and camera.IsGrabbing() and self.trackingcheckbox.state == 'down' and not stage.is_busy():
            ...
            if app.prevframe is None:
                app.prevframe = img2
            # difference image
            #ystep, xstep = macro.extractWormsDiff(app.prevframe, img2, capture_radius = -1, bin_factor = binning, minimal_difference = 0.01, dark_bg = True)
