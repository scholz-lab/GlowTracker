...
def initialize_DAQ():
    ...
    return clock, signal, out


def close_DAQ(ref_clock, ref_signal, ref_out):
    """Closes DAQ tasks

    :param ref_clock: Reference to the channel used for clocking
    :param ref_signal: Reference to the channel used to send bits
    :param ref_out: Reference to the channel used to read out info from the objective
    """
    ref_clock.close()
    ref_signal.close()
    ref_out.close()
...
def get_steps(divisor):
    """returns the hexadecimal string that makes the objective move full range in divisor number of steps"""
    max_steps = 3250  # empirical maximal number of steps
    individual_step = hex(int(max_steps/divisor))
    return individual_step


def example():
    clock, signal, out = initialize_DAQ()
    align_communication(clock, signal, out)

    command_hex = '6'
    print('For the command:' + command_hex)
    full_protocol(clock, signal, out, command_hex)
    #print(response)
    time.sleep(0.5)
    ...
    full_protocol(clock, signal, out, '6')
    time.sleep(0.5)

    complex_command = '44 00 A0'
    full_protocol(clock, signal, out, complex_command)

    close_DAQ(clock, signal, out)
