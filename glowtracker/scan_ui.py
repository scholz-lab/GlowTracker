...
def button(text, callback):
    widget = Button(text=text, size_hint_y=None, height=dp(34),
                    ...
                    background_color=(0.56, 0, 0.27, 1))
...
class ScanNumberField(BoxLayout):
    def __init__(self, panel, key, **kwargs):
        ...
        self.error = label('', 18, color=(1, 0.45, 0.5, 1))
...
def build_scan_ui(panel):
    ...
    setup_scroll = ScrollView(do_scroll_x=False)
    setup = BoxLayout(orientation='vertical', size_hint_y=None, spacing=dp(5))
    setup.bind(minimum_height=setup.setter('height'))
    setup_scroll.add_widget(setup)
    left.add_widget(setup_scroll)
    panel.bind(running=lambda _, active: setattr(setup, 'disabled', active))

    setup.add_widget(label('Plates — visited from top to bottom'))
    plate_list = GridLayout(cols=1, size_hint_y=None, spacing=dp(4))
    plate_list.bind(minimum_height=plate_list.setter('height'))
    plate_scroll = ScrollView(size_hint_y=None, height=dp(135), do_scroll_x=False)
    plate_scroll.add_widget(plate_list)
    setup.add_widget(plate_scroll)

    def refresh_plates(*args):
        plate_list.clear_widgets()
        if not panel.plates:
            plate_list.add_widget(label('Define a plate below, then add it to the run.'))
        for index, plate in enumerate(panel.plates):
            ...
            if index != panel.selected_plate:
                select.background_color = (0.25, 0.22, 0.23, 1)
            plate_list.add_widget(row(check, select))
    panel.bind(plates=refresh_plates, selected_plate=refresh_plates)
    refresh_plates()
    setup.add_widget(row(button('New plate', panel.new_plate),
                         button('Remove selected', panel.remove_plate)))
    ...
    setup.add_widget(row(button('Add / update plate', panel.store_plate),
                         button('Use current Z', panel.use_current_z)))
    ...
    setup.add_widget(row(presets, button('Load', lambda: panel.load_scenario(presets.text)),
...
def open_record_settings(panel):
    ...
    def apply(start=False):
        ...
        if panel.record_enabled and not os.path.isdir(panel.record_directory):
            ...
            destination.background_color = (1, 0.5, 0.5, 1)
            return
        popup.dismiss()
        if start:
            panel.start_run()
