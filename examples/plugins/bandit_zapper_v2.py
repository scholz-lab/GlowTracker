"""Contextual bandit for optogenetic guidance, version 2.

Same as bandit_zapper.py plus:
    early reversal check   3 s after a light stimulus the plugin checks whether the worm backed up
                           (>= `reversal_mm` along its pre-zap heading, or the app's reversal flag).
                           Logged as `reversal_check`.
    last-result state      the state is now (run age, result of the last light stimulus in this
                           wrong-way run): 'fresh' (none yet), 'ok' (it reversed) or 'fail' (it did not).
                           So the table can learn whether re-zapping after a failure pays.
    fast retry             after a failed stimulus the refractory is cut to `fail_refractory_frames`
                           (default 4 s from light-on) and the re-zap gate is lifted, so the next
                           decision can come quickly. A sham does not change the last-result state.

Everything else (arms, WAIT, sham, modes, table file, gates, status line) is as in bandit_zapper.py;
see bandit_zapper_README.md. Table keys are "<age>|<last result>|<arm>".
"""
import json
import math
import os
...
class Controller:
    ...
    decision_ages_s = (1.0, 3.0, 8.0)
    heading_window = 30
    min_travel_mm = 0.02
    refractory_frames = 360          # after a stimulus that reversed the worm (~12 s)
    fail_refractory_frames = 120     # after a stimulus that did not (~4 s from light-on)
    rezap_radius_mm = 0.5
    rezap_frames = 300
    max_pulses_per_min = 6

    # --- early reversal check --------------------------------------------------------------
    detect_frames = 90               # check 3 s after light-on
    reversal_mm = 0.10               # backed up at least this far along the pre-zap heading

    # --- state gate (roaming only) ---------------------------------------------------------
    roaming_gate = False
    min_speed_um_s = 100.0
    min_straightness = 0.5

    # --- arms, reward ----------------------------------------------------------------------
    arms = [(4.5, 45, 1, 0), (4.5, 30, 3, 30), (2.0, 45, 1, 0), (0.0, 45, 1, 0)]
    include_wait = True
    reward = 'heading'
    eval_frames = 300
    ...
    def setup(self, scope):
        ...
        self.cos_release = math.cos(math.radians(self.away_angle_deg - 15.0))
        ...
        scope.print(f'bandit v2 [{self.mode}]: {len(self.all_arms())} actions, states = age x last result, '
    ...
    def update(self, state, scope):
        ...
        for p in [p for p in self.pending if p.get('end_frame') is not None and self.frames - p['end_frame'] >= self.eval_frames]:
        ...
        threshold = self.cos_release if self.away_age > 0 else self.cos_margin
        wrong_way = cos_theta < threshold and not state.is_reversing
        if not wrong_way:
            self._reset_run()
            self.last_result = 'fresh'          # heading the right way: the episode is over
    ...
    def _reversal_check(self, scope, state):
        ...
        self.check = None
        h = c['heading']
        along = (state.worm_xy[0] - c['origin'][0]) * h[0] + (state.worm_xy[1] - c['origin'][1]) * h[1]
        reversed_ = bool(along < -self.reversal_mm) or self.reversing_seen
        self.last_result = 'ok' if reversed_ else 'fail'
        scope.log(event='reversal_check', pulse=c['pulse'], frame=self.frames, t=state.time_s, along_mm=along,
                  app_flag=self.reversing_seen, reversed=reversed_)
        if reversed_:
            scope.print(f'stimulus {c["pulse"]}: reversed ({along * 1000:+.0f} um)')
        else:
            # did nothing: allow a quick new decision instead of the full refractory
            self.cooldown = min(self.cooldown, max(0, self.fail_refractory_frames - self.detect_frames))
            self.last_zap_xy = None
            scope.print(f'stimulus {c["pulse"]}: NO reversal ({along * 1000:+.0f} um), re-arming in {self.cooldown / (state.fps or 30):.1f} s')

    # --- bandit ----------------------------------------------------------------------------
    def _state_label(self, due_age):
        return f'age {due_age:g}s|{self.last_result}'

    def _decide(self, scope, state, dist, angle, speed, straight, due_age, age_s):
        bin_label = self._state_label(due_age)
        ...
        if voltage > 0:
            heading = self._heading(state.trail)
            self.check = {'pulse': self.pulses, 'frame': self.frames, 'origin': state.worm_xy, 'heading': heading}
            self.reversing_seen = False
        scope.set_voltage(voltage)
    ...
    def _choose_arm(self, bin_label):
        ...
        if self.mode == 'test':
            tried = [(arm, s) for arm, s in stats if s['n'] > 0 and not (arm != WAIT and arm[0] == 0)]
            if not tried:
    ...
    def _status(self, scope, fps, action, angle=None, age_s=None, speed=None, straight=None, dist=None):
        if self.frames % max(1, int(fps)) != 0:
            return
        parts = [action, f'last {self.last_result}']
    ...
    def _log(self, state, scope, dist, angle, action, **extra):
        ...
        scope.log(event='track', frame=self.frames, t=state.time_s, worm=state.worm_xy, dist=dist, angle_deg=angle,
                  reversing=state.is_reversing, voltage=state.voltage, action=action, pulses=self.pulses, last=self.last_result, **extra)
